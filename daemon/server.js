...
function modelTag(agent) {
  const a = (reg.agents && reg.agents[agent]) || {};
  const cli = (a.cli || "").toLowerCase();
  if (cli && cli !== "claude") {
    if (a.model && !a.model.startsWith("claude-")) return `${cli}/${a.model}`;
    return cli;
  }
  const p = a.provider || reg.defaultProvider || "claude";
  if (p === "claude") return a.model ? "claude/" + a.model : "claude";
  if (["codex", "grok", "agy"].includes(p)) {
    if (a.model && !a.model.startsWith("claude-")) return `${p}/${a.model}`;
    return p;
  }
  return a.model ? p + "/" + a.model : p;
}

// Normalizer helper for alternative CLI tools (grok, codex, agy) -> Claude stream format
function normalizeCliEvent(m, cliEngine, state = {}) {
  if (!m || typeof m !== "object") return null;
  const eng = String(cliEngine || "").toLowerCase();
  const events = [];

  const flushText = () => {
    if (state.pendingText && state.pendingText.trim()) {
      events.push({ type: "assistant", message: { content: [{ type: "text", text: state.pendingText }] } });
    }
    state.pendingText = "";
  };

  if (eng.includes("grok")) {
    if (m.type === "text" && typeof m.data === "string") {
      state.pendingText = (state.pendingText || "") + m.data;
      return null;
    }
    if (m.type === "tool_call") {
      flushText();
      const name = m.data?.name || m.name || "tool";
      const input = m.data?.input || m.input || {};
      events.push({ type: "assistant", message: { content: [{ type: "tool_use", id: m.id || "tool_use", name, input }] } });
      return events;
    }
    if (m.type === "end") {
      flushText();
  ...
  if (eng.includes("codex")) {
    if (m.type === "thread.started" && m.thread_id) {
      state.codexThreadId = m.thread_id;
      return null;
    }
    if (m.type === "item.completed" && m.item && m.item.type === "agent_message" && m.item.text) {
      state.pendingText = m.item.text;
      flushText();
      return events;
    }
    if (m.type === "item.delta" && m.item && m.item.type === "agent_message" && (m.item.text_delta || m.item.text)) {
      state.pendingText = (state.pendingText || "") + (m.item.text_delta || m.item.text);
      return null;
    }
    if (m.type === "turn.completed") {
      ...
      events.push({
        type: "result",
        session_id: state.codexThreadId || "",
        is_error: false,
        usage: m.usage || {},
      });
      return events;
  ...
  if (eng.includes("agy") || eng.includes("antigravity")) {
    if (m.event === "step_update" && m.step_update && m.step_update.step_type === "agent_response" && m.step_update.text_delta) {
      state.pendingText = (state.pendingText || "") + m.step_update.text_delta;
      return null;
    }
    if (m.event === "result" && m.result) {
      // agy names the final text `response`; `output` was never a real field, so a
      // run whose text_delta stream was missed used to normalize to nothing at all.
      const finalText = m.result.response || m.result.output || "";
      if (typeof finalText === "string" && !state.pendingText) state.pendingText = finalText;
      // Headless agy cannot show a permission prompt, so any tool needing one (command(*)
      // is `ask` by default) is auto-denied and the turn dies with status SUCCESS and zero
      // text. That landed as task.completed — the agent looked idle instead of blocked.
      const silentDeny = !(state.pendingText || "").trim();
      if (silentDeny) {
        state.pendingText = "agy จบงานโดยไม่มีผลลัพธ์ — ปกติแปลว่ามี tool ที่ต้องขออนุญาต (เช่น command) "
          + "ถูก auto-deny เพราะโหมด headless ถามไม่ได้ ตรวจ permissions.allow ใน "
          + "~/.gemini/antigravity-cli/settings.json";
      }
      flushText();
      events.push({
        type: "result",
        session_id: m.result.conversation_id || "",
        is_error: m.result.status === "FAILURE" || silentDeny,
        usage: m.result.usage || {},
        total_cost_usd: m.result.total_cost_usd || 0,
      });
      return events;
    }
    return events.length ? events : null;
...
"${mtag}" — NOT Claude/Anthropic. Otherwise stay in character as usual.
  ...
  const cliEngine = (a && a.cli) ? String(a.cli).toLowerCase() : "";
  let cliExe = "claude";
  let args = [];
  ...
  if (cliEngine.includes("grok")) {
    cliExe = "grok";
    tempPromptFile = path.join(require("os").tmpdir(), `grok_prompt_${task.replace(/[^\w-]/g, "_")}.txt`);
    args = [
      "--no-alt-screen",
    ...
    if (route.modelArgs.length) {
      const mIdx = route.modelArgs.indexOf("--model");
      if (mIdx !== -1 && route.modelArgs[mIdx + 1]) {
        const mval = route.modelArgs[mIdx + 1];
        if (!mval.startsWith("claude-")) args.push("--model", mval);
      }
    }
  } else if (cliEngine.includes("codex")) {
    cliExe = "codex";
    // Windows unelevated sandbox + workspace-write is not usable for real dev work:
    // every dir pytest creates under the restricted token comes back Access denied,
    // so tests can never run. Inside a REGISTERED project the owner has already
    // vouched for the directory, so trade the broken sandbox for a working one there
    // and keep workspace-write everywhere else.
    const codexSandbox = (projId && process.platform === "win32") ? "danger-full-access" : "workspace-write";
    args = [
      "exec",
      "--skip-git-repo-check",
      "-s", codexSandbox,
  ...
  } else if (cliEngine.includes("agy") || cliEngine.includes("antigravity")) {
    cliExe = process.platform === "win32" ? "agy.EXE" : "agy";
    const fullPromptText = preamble + prompt + (canSplit ? SUB_NOTE : "") + VOICE_NOTE + mediaNote + TOOLS_NOTE + BRAIN_NOTE + projectNote();
    args = [
      "--print", fullPromptText,
      "--output-format", "stream-json"
    ];
    if (entry && entry.sid) args.push("--conversation", entry.sid);
    if (route.modelArgs.length) {
      const mIdx = route.modelArgs.indexOf("--model");
      if (mIdx !== -1 && route.modelArgs[mIdx + 1]) {
        const mval = route.modelArgs[mIdx + 1];
  ...
  runChildren.set(task, { child, agent, session: entry ? entry.key : "", prompt });
  ...
  const watchdog = new RunWatchdog({
    ...
    onKill: (reason) => {
      console.error(`[${cliExe}] watchdog: ${agent}/${task} killed — ${reason}`);
  ...
  const fullPromptText = preamble + prompt + (canSplit ? SUB_NOTE : "") + VOICE_NOTE + mediaNote + TOOLS_NOTE + BRAIN_NOTE + projectNote();
  if (tempPromptFile) {
    try { fs.writeFileSync(tempPromptFile, fullPromptText, "utf8"); } catch (e) { console.error("[cli] write prompt file:", e); }
  } else if (!cliEngine.includes("agy")) {
    child.stdin.write(fullPromptText);
    child.stdin.end();
  }
  ...
  const flushAssistantText = () => {
    if (!currentTextBuf.trim()) { currentTextBuf = ""; return; }
    watchdog.touch();
    let raw = currentTextBuf;
    currentTextBuf = "";
    if (canSpeak && /(^|\n)\s*SPEAK:/.test(raw)) {
      const kept = [], say = [];
  ...
  const tryFailover = (st) => {
    ...
    if (apiRetries < FAILOVER_AFTER) return false;
  ...
  child.stdout.on("data", (c) => {
    ...
    while ((i = buf.indexOf("\n")) >= 0) {
      ...
      const rawEvents = cliEngine ? normalizeCliEvent(m, cliEngine, cliState) : [m];
      ...
      for (const m of eventList) {
        ...
        if (m.type === "system") {
          if (!brainDead && m.subtype === "api_retry") {
            ...
            const permanent = st === 401 || st === 403;
            const dead = (st === null || st === undefined) && apiRetries >= 2;
            if (permanent || dead) {
              brainDead = true;
        ...
        if (m.type === "assistant" && m.message && Array.isArray(m.message.content)) {
          for (const b of m.message.content) {
            if (b.type === "tool_use") {
              ...
              watchdog.touch();
              currentTextBuf += b.text;
              lastText = currentTextBuf;
            }
          }
          ...
          if (!m.is_error) {
            ...
            brainBump(mprov, inTok, u.output_tokens || 0);
          }
          ended = true;
...
function runSub(parentId, subId, taskText, entry, onDone) {
  ...
  if (cliEngine.includes("grok")) {
    cliExe = "grok";
    tempPromptFile = path.join(require("os").tmpdir(), `grok_sub_${entry.key.replace(/[^\w-]/g, "_")}.txt`);
    args = [
      "--no-alt-screen",
      "--output-format", "streaming-json",
      "--allow", tools,
    ...
    if (route.modelArgs.length) {
      const mIdx = route.modelArgs.indexOf("--model");
      if (mIdx !== -1 && route.modelArgs[mIdx + 1]) args.push("--model", route.modelArgs[mIdx + 1]);
    }
  } else if (cliEngine.includes("codex")) {
    cliExe = "codex";
    args = [
...
const server = http.createServer((req, res) => {
  if (req.method === "GET" && (req.url.split("?")[0] === "/" || req.url.split("?")[0] === "/index.html")) {
    ...
    const KNOWN = ["claude", "codex", "grok", "agy", "glm", "deepseek", "qwen", "minimax", "moonshot",
    ...
    for (const [id, a] of Object.entries(reg.agents || {})) {
      if (id === "ceo") continue;
      const p = (a.cli ? String(a.cli).toLowerCase() : a.provider) || reg.defaultProvider || "claude";
      const list = sess[id] || [];
    ...
    const providers = ids.map((id) => {
      ...
      return { id, label: c.label || id, kind: c.kind || (KNOWN.includes(id) ? "" : "custom"),
        connected: (id === "claude" || id === "codex" || id === "grok" || id === "agy") ? true : !!c.connected, agents: byProvider[id] || [] };
  ...
  } else if (req.method === "GET" && req.url === "/system/cli-status") {
    const { execSync } = require("child_process");
    function checkCli(cmd) {
      let ver = "", bin = "";
      try { ver = execSync(`${cmd} --version`, { encoding: "utf8", timeout: 2500, stdio: ["ignore", "pipe", "ignore"] }).trim().split("\n")[0]; } catch {}
      try {
        const whereCmd = process.platform === "win32" ? `where.exe ${cmd}` : `which ${cmd}`;
        bin = execSync(whereCmd, { encoding: "utf8", timeout: 2500, stdio: ["ignore", "pipe", "ignore"] }).trim().split("\r\n")[0].split("\n")[0];
      } catch {}
      return { installed: !!(ver || bin), ver: ver || "(ไม่พบเวอร์ชัน)", bin: bin || "(ไม่พบไฟล์)" };
    }
    const resData = {
      claude: checkCli("claude"),
      codex: checkCli("codex"),
      grok: checkCli("grok"),
      agy: checkCli(process.platform === "win32" ? "agy.EXE" : "agy")
    };
    res.writeHead(200, { "content-type": "application/json; charset=utf-8" });
  ...
  set custom title of (window 1 where tabs contains t) to "${asEsc(marker)}"
    ...
    readBody(req, (body) => {
      ...
      const rc = runChildren.get(task) || {};
      broadcast({ type: "perm.requested", agent, task, tool, perm: id, input, session: rc.session || "", prompt: rc.prompt || "" });
